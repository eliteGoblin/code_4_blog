# TODO: Use the original gem package once this PR is released: https://github.com/graylog-labs/gelf-rb/pull/53
# The original source of the plugin is from https://github.com/emsearcy/fluent-plugin-gelf/blob/master/lib/fluent/plugin/out_gelf.rb
# But default fluentd collects logs in ascii 8 bits format, but gelf supports UTF-8
# We manually modified string entries to enforce UTF-8
module Fluent
    ...
    class GELFOutput < BufferedOutput
      ...
      def format(tag, time, record)
        ...
        record.each_pair do |k,v|
          ...
          ###### CUSTOM CODE TO FIX ENCODING ####
          when 'short_message', 'full_message', 'facility', 'line', 'file' then
            gelfentry[k.to_sym] = v.is_a?(String) ? v.force_encoding(Encoding::UTF_8) : v
          else
            gelfentry["_#{k}".to_sym] = v.is_a?(String) ? v.force_encoding(Encoding::UTF_8) : v
          end
          ###### END OF CUSTOM CODE TO FIX ENCODING ####
        end
